...
public class Server2 {
	...
	public static void main(String[] args) {
		...
		System.out.println("Starting  - eBay");
		System.out.println("Success!");
		try {
			c = Cipher.getInstance("RSA");
			server = new ServerSocket(port);
			// Get public key
			byte[] publickeyServerBytes = Files.readAllBytes(new File("F:\\NS Project\\publickey_eBay_server.txt").toPath());
			...
			System.out.println("Generating eBay's public key...");
			System.out.println("Success!");
			System.out.println("eBay's public key is : " + pub_server);

			// Get private key
			byte[] privatekeyBytes = Files.readAllBytes(new File("F:\\NS Project\\privatekey_eBay_server.txt").toPath());
			...
			while (true) {
				// Building connection
				System.out.println("eBay Waiting for the broker's request...");
				...
				dos.flush();
				System.out.println("eBay Sending Public Key to the broker...");
				...
				if (!names[0].equals("PayPal")) {
					...
					dos.flush();
					System.out.println("Timer expired.  eBay Closing connection...");
					s.close();

				} else {
					dos.writeUTF("Success!");
					System.out.println(
							"eBay Succesfully authenticated the broker. Getting session key from the broker...");

				}

				// Step 4 of phase 2: Prove myself
				System.out.println("eBay Authenticating to the broker...");
				int i = 0;
				Random r = new Random();
				...
				data_to_encrypt = session_string + "," + "eBay";
				...
				if (!names[0].equals("PayPal") || !names[1].equals("send the list")) {
					System.out.println("Invalied request received.. Closing Connection");
					dos.writeUTF("Not PayPal");
					dos.flush();
					s.close();
				} else {
					dos.writeUTF("Success!");
					System.out.println(" eBay Received a request from the broker to send my list...");

				}

				// Step 6 of phase 2: Send information to broker
				System.out.println("eBay sending product list to the broker...");
				c = Cipher.getInstance("AES");
				productlist = readCSV("F:\\NS Project\\products\\productlist_eBay.csv");
				String time_stamp2 = sdf.format(new Date());
				data_to_encrypt = "eBay," + time_stamp2;
				to_send = data_to_encrypt.getBytes();
				c = Cipher.getInstance("RSA");
				c.init(Cipher.ENCRYPT_MODE, pri_server);
				encryptedData = c.doFinal(to_send);
				...
				System.out.println("eBay receiving information from the broker...");
				c = Cipher.getInstance("AES");
				c.init(Cipher.DECRYPT_MODE, n2);
				len = dis.readInt();
				b = new byte[len];
				...
				System.out.println(
						"eBay Received purchase information from the broker with amount of " + amount + " dollars...");
				System.out.println("Success");

				// Step 2 of phase 4: Send message to Broker.
				System.out.println("eBay sending product to the broker...");
				String product = "";
				if (product_ID.equals("1")) {
					product = readFile("F:\\NS Project\\products\\product1_eBay.txt");

				}
				if (product_ID.equals("2")) {
					product = readFile("F:\\NS Project\\products\\product2_eBay.txt");

				}
				if (product_ID.equals("3")) {
					product = readFile("F:\\NS Project\\products\\product3_eBay.txt");

				}
				if (product_ID.equals("4")) {
					product = readFile("F:\\NS Project\\products\\product4_eBay.txt");

				}

				data_to_encrypt = product + "," + "eBay";
				...
				dos.writeInt(encryptedData.length);
				dos.write(encryptedData);
				System.out.println("eBay Sent the product to the client via the broker.");
